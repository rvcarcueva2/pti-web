...
export default function PlayersPage() {
  ...
  const columns = [
    ...
    { label: 'Group', key: 'group', width: 'w-[170px]' },
  ];
  ...
  const handleExportExcel = async () => {
    try {
      ...
      filteredPlayers.forEach((player, index) => {
        ...
        setCell('A', player.lastName);
        setCell('B', player.firstName);
        setCell('C', player.sex);
        setCell('D', parseInt(player.age));
        setCell('E', parseFloat(player.height));
        setCell('F', parseFloat(player.weight));
        setCell('G', player.belt);
        setCell('H', player.level);
        setCell('I', player.group);
      });

      // 3. Update the worksheet range to include all data
      if (filteredPlayers.length > 0) {
        const lastRow = filteredPlayers.length + 1;
        worksheet['!ref'] = `A1:I${lastRow}`;
      }
  ...
  }, [newPlayer.age, newPlayer.height, newPlayer.weight, newPlayer.sex, newPlayer.categories, newPlayer.belt]);
    ...
    <div className="font-geist p-6 ml-64">
      ...
      <div className="mb-4">
        <div className="flex items-center gap-2 mb-2">
          <Link href="/admin-panel/competitions/competition" className="font-medium group text-md text-[#EAB044] flex items-center">
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="pr-4">
              <button 
                ...
                className={`cursor-pointer flex items-center gap-2 px-4 py-2 text-sm rounded transition-colors ${
                  hasActiveFilters 
