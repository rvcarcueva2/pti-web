...
export default function RegisterModal({ competitionId }: { competitionId: string }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!teamId || !userEmail || !competitionId || players.length === 0) {
      alert("Missing required data or no players found.");
      return;
    }
    ...
    <>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/30 flex justify-center items-center z-50">
          <div className="bg-white rounded-lg shadow-lg p-6 w-115 relative max-h-[90vh] overflow-y-auto">
            ...
            {isLoading ? (
              ...
              <>
                <input
                  ...
                  value={teamName}
                  disabled
                  className="w-full px-4 py-2.5 rounded-lg bg-gray-200 border"
                />
                {isDuplicate && (
                ...
                <div className="mt-3 flex items-start gap-3">
                  <input
                    type="checkbox"
                    id="agree"
                    className={`mt-1 accent-black ${agreeError ? 'ring-2 ring-red-500' : ''}`}
                ...
                <button
                  ...
                  className="mt-4 px-4 py-2 rounded bg-[#EAB044] text-white hover:bg-[#d49a35] cursor-pointer"
                >
                  Confirm
                </button>
              </>
